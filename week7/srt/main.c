...
int nthreads;
scene_t scene;
float thread_color[width][height][3];
...
for( int px=*numofthreads; px<width; px+=nthreads)
    {
	    ...
	    thread_color[px][py][0]=scaled_color[0];
	    thread_color[px][py][1]=scaled_color[1];
	    thread_color[px][py][2]=scaled_color[2];
	}
    }
 return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for(i=0;i<nthreads;i++)
      {
        thread_num[i]=i;
	r=pthread_create(&threads[i],NULL,threadfunc,&thread_num[i]);
	if(r)
	  {
    ...
    for(i=0;i<width;i++){
      for(j=0;j<height;j++){
          printf( "%.0f %.0f %.0f\n",
		    thread_color[i][j][0], thread_color[i][j][1], thread_color[i][j][2]);
        }
        printf( "\n" );
    }
